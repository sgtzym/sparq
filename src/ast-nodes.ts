...
import { needsQuoting, sql, type SqlString, toSqlValue } from '~/core/sql.ts'
...
export class LiteralNode implements Node {
    ...
    render(params: Parameters): SqlString {
        return params.add(toSqlValue(this.value))
    }
}
...
export class IdentifierNode implements Node {
    ...
    render(_params: Parameters): SqlString {
        const sql: string = this.name.split('.')
...
export class ConjunctionNode implements Node {
    ...
    render(params: Parameters): SqlString {
        ...
        const conditions: string = renderAll(this.conditions, params)
            .join(` ${op} `)
...
export class ValueListNode implements Node {
    ...
    render(params: Parameters): SqlString {
        const values: string = renderAll(this.values, params)
            .join(', ')
...
export class FromNode implements Node {
    ...
    render(params: Parameters): SqlString {
        const tables: string = renderAll(this.tables, params)
            .join(', ')
...
export class ValuesNode implements Node {
    constructor(
        private readonly rows: ArrayLike<Node>,
    ) {}
...
export class SetNode implements Node {
    ...
    render(params: Parameters): SqlString {
        const assignments: string = renderAll(this.assignments, params)
            .join(', ')
